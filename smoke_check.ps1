...
python -c "import uuid; from fastapi.testclient import TestClient; from app import app; c=TestClient(app); e=f'smoke_{uuid.uuid4().hex[:8]}@example.com'; p='StrongPass123'; c.post('/api/v1/auth/register', json={'email':e,'password':p,'full_name':'Smoke User'}); t=c.post('/api/v1/auth/login', json={'email':e,'password':p}).json()['access_token']; h={'Authorization':f'Bearer {t}'}; k=c.post('/api/v1/integrations/api-key', json={'api_key':'sk-smoke-key-123456789'}, headers=h); rr=c.post('/api/v1/chat/route', json={'prompt':'Smoke test prompt'}, headers=h); ok='yes' if rr.status_code==200 and k.status_code==200 else 'no'; print('api_key:', k.status_code, 'chat_route:', rr.status_code, 'ok:', ok)"
